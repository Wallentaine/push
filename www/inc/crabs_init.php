...
/*
*
* @ This file is created by https://DeCodez.Net
* @ DeCodeZ (Best Decoder for PHP Files)
*
* @ Version              :    7.2.0.0
* @ Author               :    DeCodeZ
* @ Release on           :    21.01.2024
* @ Official site        :    https://DeCodez.Net
*
*/

// $filename = dirname(__FILE__) . '/conf.php';
// $cfg_txt = fopen($filename, 'r') or exit('Ошибка лицензии. Помощь на @mister_crabs');
...
$connect_db = mysqli_connect(DB_HOST, DB_USER, DB_PASS, DB_BASE) or exit('Error: ' . mysqli_connect_error());
...
mysqli_set_charset($connect_db, DB_CHARSET) or exit('Error');
...
if (filter_var($ip_client, FILTER_VALIDATE_IP)) {
	$ip = $ip_client;
} else if (filter_var($ip_forward, FILTER_VALIDATE_IP)) {
	$ip = $ip_forward;
} else {
	$ip = $ip_remote;
}
...
function wudate($time = 0, $bletter = 1)
{
	...
	$MN = ['воскресенье', 'понедельник', 'вторник', 'среда', 'четверг', 'пятница', 'суббота'];
...
if ($minused < 60) {
		return $minused . ' ' . wu_end($minused, ['секунду', 'секунды', 'секунд']) . ' назад';
	} else if ($minused < 3600) {
		return round($minused / 60) . ' ' . wu_end(round($minused / 60), ['минуту', 'минуты', 'минут']) . ' назад';
	} else if ($minused < 86400) {
		return round($minused / 3600) . ' ' . wu_end(round($minused / 3600), ['час', 'часа', 'часов']) . ' назад';
	} else if ($minused < 172800) {
		if (1 == $bletter) {
			$pre = 'Вчера';
		} else {
			$pre = 'вчера';
...
function crabs_protocol()
{
	...
	if (isset($_SERVER['HTTPS']) && 'on' == $_SERVER['HTTPS']) {
		$isSecure = 'https://';
	} else if (!empty($_SERVER['HTTP_X_FORWARDED_PROTO']) && 'https' == $_SERVER['HTTP_X_FORWARDED_PROTO'] || !empty($_SERVER['HTTP_X_FORWARDED_SSL']) && 'on' == $_SERVER['HTTP_X_FORWARDED_SSL']) {
		$isSecure = 'https://';
	}
