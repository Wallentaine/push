...
<div class="form-group">
<label for="pnamez">Название проекта:</label>
<input type="text" class="form-control" id="pnamez" placeholder="Название проекта" value="<?php if ($nowusr['ty'] != 0) { echo htmlspecialchars($data['project_name']); } else { echo 'Скрыто в демо аккаунте'; } ?>">
</div>

<div class="form-group">
<label for="srvk">Имя приватного ключа:</label>
<input type="text" class="form-control" id="srvk" placeholder="Имя приватного ключа" value="<?php if ($nowusr['ty'] != 0) { echo htmlspecialchars($data['private_key']); } else { echo 'Скрыто в демо аккаунте'; } ?>">
</div>
...
var pnamez = $('#pnamez').val();
var srvk = $('#srvk').val();
$.ajax({
type: 'POST',
url: '/actions/admin_data_pdomain_save.php',
data: {'domain': domain, 'sendid': sendid, 'pnamez': pnamez, 'srvk': srvk, 'token': crabs_tkn},
cache: false,
success: function(result){
if (result == '0') {
$.jGrowl('Заполните все поля', { theme: 'growl-error' });
