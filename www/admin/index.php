...
while($rowi = mysqli_fetch_assoc($statq)) {
    if ($rowi['n_clk'] > 0) { $iicr = number_format($rowi['n_subs']/$rowi['n_clk']*100,2,'.',''); } else { $iicr = 0;}
    if ($rowi['o_shows'] > 0) { $iictr = number_format($rowi['o_clk']/$rowi['o_shows']*100,2,'.',''); } else { $iictr = 0;}
...
$timeline[$sdt]['cr'] = $iicr;
$timeline[$sdt]['ctr'] = $iictr; if ($timeline[$sdt]['ctr'] == 'nan' || $timeline[$sdt]['ctr'] == 'inf') { $timeline[$sdt]['ctr'] = '0.00'; }
}
...
<div class="card-body">
<h4 class="card-title text-center"><?php if ($data['p_all_showed'] > 0) { echo number_format($data['p_all_clicks']/$data['p_all_showed']*100,2,'.',','); } else { echo 0; } ?></h4>
</div>
</div>
</div>
