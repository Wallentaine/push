...
$pnamez = mysqli_real_escape_string($connect_db, trim($_POST['pnamez']));
$srvk = mysqli_real_escape_string($connect_db, trim($_POST['srvk']));

mysqli_query($connect_db, "UPDATE `t_data` SET `pdomain` = '$domain', `sender_id` = '$sendid', `project_name` = '$pnamez', `private_key` = '$srvk' LIMIT 1");
exit('1');
} else { exit('0'); }
?>